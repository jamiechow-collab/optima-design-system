...
// Colours sampled/extracted from the Figma file
const C = {
  fill: '#A5B0C0', // grey-200 — columns + nav rail
  navIcon: '#72839C', // grey-400 — nav rail icon squares
  logo: '#1F1F1F',
  canvasBorder: '#E3E6EC',
  indicator: '#3758F9', // blue measurement brackets
  marginBracket: '#9CA3AF', // grey ⊔ bracket under the edge gap
  badgeBg: '#EFF3FF',
  badgeText: '#3758F9',
  label: '#1F2937',
  titleSub: '#8D9BAF', // grey-300 — "(1728 px)" part of the heading
};
...
const UBracket = ({ left, width, top = 0 }) => (
  <div
    style={{
      ...
      top,
      height: 8,
      borderLeft: `1px solid ${C.marginBracket}`,
      borderRight: `1px solid ${C.marginBracket}`,
      borderBottom: `1px solid ${C.marginBracket}`,
    }}
  />
);
...
const BreakpointBlock = ({ spec }) => {
  ...
  const labelFont = s(columnWidth + gutter) < 36 ? 9 : 11;

  return (
    <div style={{ marginBottom: 80 }}>
      {/* Title — "Desktop" bold + "(1728 px)" grey */}
      <div style={{ fontSize: 18, fontWeight: 700, color: '#111827', marginBottom: 20 }}>
        {title} <span style={{ color: C.titleSub }}>{sub}</span>
      </div>
      ...
      {/* Canvas: white panel with side-nav rail + columns */}
      <div
        style={{
          position: 'relative',
          width: W,
      ...
      {/* Gutter guide: I-beam over each gutter + bold px label */}
      ...
      {/* Edge-gap measurement: ⊔ bracket + badge at both canvas edges */}
...
export const AllBreakpoints = () => (
  <Page>
    <h1 style={{ fontSize: 28, fontWeight: 700, margin: '0 0 8px', color: '#111827' }}>
      Layout grid
    </h1>
    <p style={{ fontSize: 14, lineHeight: 1.6, color: '#374151', margin: '0 0 48px', maxWidth: 660 }}>
